from django.shortcuts import render
from django.views.decorators.csrf import csrf_exempt
from django.http import JsonResponse
from django.core.paginator import Paginator

import pandas as pd
from sklearn.feature_extraction.text import TfidfVectorizer
from sklearn.neighbors import NearestNeighbors

from .models import Book
import os
from django.conf import settings
import json

# ============================
# LOAD DATASET
# ============================
CSV_PATH = os.path.join(settings.BASE_DIR, "final_cleaned_books_updated2.csv")
books_df = pd.read_csv(CSV_PATH)

# Prepare first 100 books for API page
books_page_df = books_df.head(100)
books_list_for_page = books_page_df[["book_title", "book_author", "year_of_publication", "image_url"]].to_dict(orient="records")

# Fill missing publisher values
books_df["publisher"] = books_df["publisher"].fillna("N/A")

# Combine features for TF-IDF
books_df["combined_features"] = (
    books_df["book_title"].astype(str) + " " +
    books_df["book_author"].astype(str) + " " +
    books_df["publisher"].astype(str)
)

# TF-IDF Vectorization
tfidf = TfidfVectorizer(stop_words="english", max_features=5000, ngram_range=(1, 2), min_df=2)
tfidf_matrix = tfidf.fit_transform(books_df["combined_features"])

# Nearest Neighbors Model
nn_model = NearestNeighbors(n_neighbors=6, metric="cosine", algorithm="brute")
nn_model.fit(tfidf_matrix)

# ============================
# RECOMMENDATION FUNCTION
# ============================
def get_similar_books(title, num_recommendations=5):
    matches = books_df[books_df["book_title"].str.lower().str.strip() == title.lower().strip()]
    
    if matches.empty:
        return []

    book_index = matches.index[0]

    try:
        distances, indices = nn_model.kneighbors(tfidf_matrix[book_index], n_neighbors=num_recommendations + 1)
    except IndexError:
        return []

    recommendations = []
    seen_titles = set()

    for idx, score in zip(indices[0][1:], distances[0][1:]):
        if idx >= len(books_df):
            continue

        book_info = books_df.iloc[idx]
        book_title = book_info["book_title"]

        if book_title in seen_titles:
            continue

        recommendations.append({
            "title": book_info["book_title"],
            "author": book_info["book_author"],
            "publisher": book_info["publisher"],
            "similarity_score": round((1 - score) * 100, 2),
            "image_url": book_info["image_url"] or "/static/images/default-cover.jpg"
        })

        seen_titles.add(book_title)

        if len(recommendations) == num_recommendations:
            break

    return recommendations

# ============================
# PAGE VIEWS
# ============================
def home(request):
    return render(request, "home.html")

def books_page(request):
    all_books = Book.objects.only('book_title', 'book_author', 'image_url', 'year_of_publication')
    paginator = Paginator(all_books, 20)
    page_number = request.GET.get('page')
    page_books = paginator.get_page(page_number)
    return render(request, "books.html", {"page_books": page_books})

def all_books(request):
    """Show all books without pagination"""
    all_books = Book.objects.all()
    return render(request, "all_books.html", {"books": all_books})

def contact(request):
    return render(request, "contact.html")

def login_view(request):
    return render(request, "login.html")

def recommendations_page(request):
    return render(request, "recommendations.html")

def books_list(request):
    return JsonResponse({"books": books_list_for_page})

def chatbot_page(request):
    return render(request, "chatbot.html")

# ============================
# API ENDPOINTS
# ============================
def recommend_book(request):
    if request.method == "GET":
        book_title = request.GET.get("book_title", "").strip()
        if not book_title:
            return JsonResponse({"error": "Book title is required"}, status=400)

        recommendations = get_similar_books(book_title)
        if not recommendations:
            return JsonResponse({"error": "Book not found."}, status=404)

        return JsonResponse({"recommendations": recommendations})

def check_book_exists(request):
    title = request.GET.get("title", "")
    exists = not books_df[books_df["book_title"].str.lower().str.strip() == title.lower().strip()].empty
    return JsonResponse({"exists": exists})

def search_results(request):
    query = request.GET.get("query", "").strip()
    matched_book = books_df[books_df["book_title"].str.lower().str.strip() == query.lower()]

    if matched_book.empty:
        return render(request, "search.html", {"book": None})

    book_data = matched_book.iloc[0]

    return render(request, "search.html", {
        "book": {
            "title": book_data["book_title"],
            "author": book_data["book_author"],
            "publisher": book_data["publisher"],
            "year": book_data["year_of_publication"],
            "image_url": book_data["image_url"] or "/static/images/default-cover.jpg"
        }
    })

def search_suggestions(request):
    query = request.GET.get("query", "").lower()
    suggestions = []

    if query:
        matched_books = books_df[books_df["book_title"].str.lower().str.contains(query, na=False)]
        suggestions = matched_books["book_title"].dropna().unique().tolist()[:10]

    return JsonResponse({"suggestions": suggestions})

# ============================
# CHATBOT API
# ============================
@csrf_exempt
def chatbot_api(request):
    if request.method != "POST":
        return JsonResponse({"error": "POST required"}, status=400)

    try:
        data = json.loads(request.body)
        user_message = data.get("message", "").strip().lower()

        if not user_message:
            return JsonResponse({"response": "Please type something!"})

        # Greetings
        if user_message in ["hi", "hello", "hey"]:
            return JsonResponse({"response": "Hello! 👋 How can I assist you today?"})

        # RECOMMENDATION HANDLING
        if "recommend" in user_message:
            parts = user_message.split("recommend", 1)
            book_title = parts[1].strip() if len(parts) > 1 else ""

            if not book_title:
                return JsonResponse({"response": "Please tell me the book name.\nExample: recommend Harry Potter"})

            recommendations = get_similar_books(book_title)

            if not recommendations:
                return JsonResponse({
                    "response": f"❌ No books found similar to **{book_title.title()}**."
                })

            html_response = f"""
            <div style="font-size: 16px; margin-bottom: 10px;">
                <strong>📚 Books similar to <span style='color:#4f46e5'>{book_title.title()}</span>:</strong>
            </div>
            """
            for rec in recommendations:
                html_response += f"""
                <div style="display: flex; align-items: center; margin-bottom: 12px; background:#f3f4f6; padding:10px; border-radius:8px;">
                    <img src="{rec['image_url']}" 
                        alt="Cover" 
                        style="width:60px; height:90px; object-fit:cover; border-radius:5px; margin-right:12px;">
                    <div>
                        <div style="font-size: 16px; font-weight: bold;">{rec['title']}</div>
                        <div style="font-size: 14px; color:#555;">by {rec['author']}</div>
                    </div>
                </div>
                """

            return JsonResponse({"response": html_response})

        # Default fallback
        return JsonResponse({"response": "I’m not sure I understand 🤖. Try saying:\n<b>recommend Harry Potter</b>"})

    except Exception as e:
        print("Chatbot error:", e)
        return JsonResponse({"response": "⚠️ Server error. Try again!"})
